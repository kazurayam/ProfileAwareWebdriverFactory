...
class FirefoxProfileUtils {

    static Path getDefaultUserDataDir() {
        if (OSIdentifier.isWindows()) {
            ...
            return Paths.get(System.getProperty('user.home')).resolve('Library').
                    resolve('Application Support').resolve('Firefox').resolve('Profiles')
        } else if (OSIdentifier.isUnix()) {
            // ~/.mozilla/firefox
            return Paths.get(System.getProperty('user.home')).resolve('.mozilla').
                    resolve('firefox')
        } else {
    ...
    static List<FirefoxUserProfile> getFirefoxUserProfileList() {
...
drwx------@ 48 kazuakiurayama  staff  1536  4  1  2021 yqym488l.default-release
        ...
        cookies.sqlite-wal
     */
    static List<FirefoxUserProfile> getFirefoxUserProfileList(Path userDataDir) {
        ...
        for (Path dir : dirs) {
            ...
            FirefoxUserProfile fup =
                    new FirefoxUserProfile(userDataDir,
